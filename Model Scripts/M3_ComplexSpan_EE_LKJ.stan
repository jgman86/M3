...
transformed parameters{
  ...
  for(j in 1:Con) {
    ...
    acts_IOP[j + (i-1)*Con] = scale_b + (1+ subj_pars[i,4]*Freetime[j])* subj_pars[i,1];        // Item in Other Position
    acts_DIP[j + (i-1)*Con] = scale_b + exp(-subj_pars[i,5]*Freetime[j])* f*(subj_pars[i,2]+ subj_pars[i,1]);// Distractor in Position
    acts_DIOP[j + (i-1)*Con] = scale_b + exp(-subj_pars[i,5]*Freetime[j])* f* subj_pars[i,2]; // Distractor in other Position

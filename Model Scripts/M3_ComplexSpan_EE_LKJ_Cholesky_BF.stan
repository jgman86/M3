...
data {
  ...
  real fixed_f; // fix the f parameter
...
// real mu_f = inv_logit(hyper_pars[3]);
  ...
  for(j in 1:Con) {
    ...
    acts_IIP[j + (i-1)*Con] = scale_b + (subj_pars[i,2] + ((1+ subj_pars[i,3]*Freetime[j])*subj_pars[i,1])); // Item in Position                      
   // acts_IOP[j + (i-1)*Con] = scale_b + (1+ subj_pars[i,4]*Freetime[j])* subj_pars[i,2];        // Item in Other Position
    ...
    acts_DIP[j + (i-1)*Con] = scale_b + fixed_f*(exp(-subj_pars[i,4]*Freetime[j])*subj_pars[i,1] + subj_pars[i,2]); // Distractor in Position
    acts_DIOP[j + (i-1)*Con] = scale_b + fixed_f* subj_pars[i,2]; // Distractor in other Position
